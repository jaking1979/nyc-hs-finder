...
export function labelAdmissions(s: string | undefined) {
  ...
  if (t.includes("edopt") || t.includes("educational option")) return "Educational Option";
  if (t.includes("open")) return "Open";
  if (t.includes("zone")) return "Zoned";
  return "Other";
}

export function renderProgramMeta(
  p: ScoredProgram & { admissionsMethod?: string; tags?: string[] }
) {
  const lines: string[] = [];
